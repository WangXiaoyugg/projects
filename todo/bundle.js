...
var app = new _vue2.default({
    ...
    created: function created() {
        ...
        window.onbeforeunload = function () {
            var inputDataStr = JSON.stringify(_this.newTodo);
            window.localStorage.setItem('myNewTo', inputDataStr);

            var signDataStr = JSON.stringify(_this.msg);
            window.localStorage.setItem('mySign', signDataStr);
        };

        var oldInputStr = window.localStorage.getItem('myNewTo');
        var oldInputData = JSON.parse(oldInputStr);
        this.newTodo = oldInputData || '';

        var oldSign = window.localStorage.getItem('mySign');
        var oldSingData = JSON.parse(oldSign);
    ...
    methods: {
        ...
        updateTodos: function updateTodos() {
            var listDataStr = JSON.stringify(this.todoList);
        ...
        signUp: function signUp() {
            ...
            user.signUp().then(function (loginedUser) {
                ...
                _this4.msg = loginedUser.getUsername();
            }, function (error) {
                alert('注册失败');
            });
        },
        login: function login() {
            ...
            _leancloudStorage2.default.User.logIn(this.formData.username, this.formData.password).then(function (loginedUser) {
                ...
                _this5.msg = _this5.formData.username;
                _this5.fetchTodos();
            }, function (error) {
                alert('登录失败');
            });
...
function setTime(date) {
    ...
    var time = year + '年' + month + '月' + day + '日' + hour + '时' + min + '分';
    return time;
}
